...
import { api, FileDetail, FileItem, Folder as FolderType, Stats, User } from './api'
...
export default function VaultWorkspace() {
  ...
  const signIn = async (e: React.FormEvent) => { e.preventDefault(); try { const u = await api.login(email, password); const c = await api.csrf(); setCsrf(c.csrfToken); setUser(u) } catch (e) { setError(e instanceof Error ? e.message : 'Unable to sign in') } }
  ...
  if (!user) return <main className="auth"><div className="auth-card"><div className="brand"><ShieldCheck /> BalkanID Vault</div><p className="eyebrow">SECURE FILE OPERATIONS</p><h1>Your private workspace.</h1><form onSubmit={signIn}><label>Email<input value={email} onChange={e => setEmail(e.target.value)} type="email" /></label><label>Password<input value={password} onChange={e => setPassword(e.target.value)} type="password" /></label>{error && <p className="error">{error}</p>}<button className="primary">Sign in to vault</button></form></div></main>

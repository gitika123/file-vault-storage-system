...
\draw[arr] (ui) -- node[above=4pt,fill=white,inner sep=2pt,font=\scriptsize]{same-origin HTTPS} (api);
\draw[arr] (api) -- (db);
\draw[arr] (api) -- (blob);
\draw[arr] (api) -- (redis);
\end{tikzpicture}
...
Reusable prompt patterns and the complete methodology are recorded in \code{docs/ai-prompts.md} and \code{docs/ai-methodology.md}. No production credentials, tokens, private files, or secret values are included in source control.

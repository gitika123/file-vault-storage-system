...
\vspace{0.22in}
\fcolorbox{cyan}{navy}{\parbox{0.82\textwidth}{\centering
\textcolor{cyan}{\textbf{\small LIVE APPLICATION}}\par\vspace{3pt}
\textcolor{white}{\large\url{https://frontend-damj.onrender.com/}}\par
\vspace{3pt}\textcolor{cyan}{\scriptsize API health: \url{https://api-1gm3.onrender.com/health/ready}}}}
\end{center}\vfill
...
\textbf{Live access} & \url{https://frontend-damj.onrender.com/}\\
...
\textit{Read the product brief and turn it into a clear, dependency-ordered checklist. Mark each item as core, deliverable, bonus, or blocked. For every item, define what a reviewer should be able to observe and how we will verify it.}

\item[Backend implementation]
\textit{Implement this requirement in the existing Go module. Keep the current API behavior, ownership rules, configuration patterns, structured errors, parameterized SQL, and transaction boundaries intact. Add focused tests for the normal path, failures, authorization, boundaries, and concurrency. Keep unrelated modules unchanged.}

\item[Database and scale review]
\textit{Review this schema or query for ownership leaks, race conditions, rollback problems, incorrect reference counts, and missing index support. Recommend the smallest forward-only change. Explain how tests or an execution plan will prove that the change is safe and useful at scale.}

\item[Frontend implementation]
\textit{Build this user journey in the existing React and TypeScript design system. Account for loading, empty, error, success, keyboard, responsive, and authorization states. Keep API calls typed and add a browser test for the most important user action.}

\item[Security review]
\textit{Review authentication, CSRF protection, authorization, file paths, MIME checks, token handling, security headers, rate limiting, and error messages. For each issue, show the evidence, explain the risk, describe a realistic exploit, and recommend the smallest safe fix.}

\item[Final verification]
\textit{Check the implementation against every acceptance criterion. Run focused checks first, then the full verification suite. Record the exact commands and results. Separate verified behavior from assumptions, and flag any documentation statement that the available evidence does not support.}
\end{description}

People made the final decisions about scope, code, security, tests, and documentation. The repository contains no production credentials, tokens, private files, or secret values.
